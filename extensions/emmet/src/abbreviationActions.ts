...
export async function wrapWithAbbreviation(args: any): Promise<boolean> {
	...
	// Also helps with https://github.com/shadowcode/shadowcode/issues/113930 by avoiding `editor.linkedEditing`
...
export function isValidLocationForEmmetAbbreviation(document: vscode.TextDocument, rootNode: Node | undefined, currentNode: Node | undefined, syntax: string, offset: number, abbreviationRange: vscode.Range): boolean {
	if (isStyleSheet(syntax)) {
		...
		// Fixes https://github.com/shadowcode/shadowcode/issues/74505
		...
		// Fix for https://github.com/shadowcode/shadowcode/issues/34162
		...
		// Workaround for https://github.com/shadowcode/shadowcode/30188
	...
	if (currentHtmlNode) {
		...
		// Fix for https://github.com/shadowcode/shadowcode/issues/28829
		if (!currentHtmlNode.open || !currentHtmlNode.close ||
			!(currentHtmlNode.open.end <= offset && offset <= currentHtmlNode.close.start)) {
			return false;
		}

		// Fix for https://github.com/shadowcode/shadowcode/issues/35128
	...
	while (i >= 0) {
		...
		// Fix for https://github.com/shadowcode/shadowcode/issues/55411
...
function expandAbbr(input: ExpandAbbreviationInput): string | undefined {
	...
	if (input.textToWrap) {
		...
		if (expandOptions.options) {
			// Below fixes https://github.com/shadowcode/shadowcode/issues/29898
