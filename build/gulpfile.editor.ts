...
const BUNDLED_FILE_HEADER = [
	...
	' * https://github.com/shadowcode/shadowcode/blob/main/LICENSE.txt',
	' *-----------------------------------------------------------*/',
	''
].join('\n');
...
const finalEditorResourcesTask = task.define('final-editor-resources', () => {
	return es.merge(
		...
		gulp.src('build/monaco/version.txt')
			.pipe(es.through(function (data) {
				data.contents = Buffer.from(`monaco-editor-core: https://github.com/shadowcode/shadowcode/tree/${sha1}`);
				this.emit('data', data);
			}))
			.pipe(gulp.dest('out-monaco-editor-core')),
