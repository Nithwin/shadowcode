---
name: add-policy
description: Use when adding, modifying, or reviewing ShadowCode configuration policies. Covers the full policy lifecycle from registration to export to platform-specific artifacts. Run on ANY change that adds a `policy:` field to a configuration property.
---
...
Check the `ShadowCode - Build` watch task output, or run:
