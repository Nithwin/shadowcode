...
Fixtures render inside shadow DOM. The component-explorer automatically adopts the global ShadowCode stylesheets and theme CSS.

### Matching production CSS selectors

Many ShadowCode components have CSS rules scoped to deep ancestor selectors (e.g., `.interactive-session .interactive-input-part > .widget-container .my-element`). In fixtures, you must recreate the required ancestor DOM structure for these selectors to match:
