...
vs/workbench     ← Standard ShadowCode workbench
...
### 2.3 How It Differs from ShadowCode

| Aspect | ShadowCode Workbench | Agent Sessions Window |
|--------|-------------------|----------------------|
...
Views and contributions that should only appear in the agent sessions window (not in regular ShadowCode) use `WindowVisibility.Sessions` in their registration.
...
2. Lean on existing ShadowCode services (`IPromptsService`, `IMcpService`, `IChatService`)
3. Browser compatibility required — no Node.js APIs
4. Active worktree comes from `IActiveSessionService`

### 10.5 Validation

1. Check `ShadowCode - Build` task output for compilation errors before declaring work complete
