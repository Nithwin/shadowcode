---
name: Demonstrate
description: Agent for demonstrating ShadowCode features
...
2. Search the `shadowcode/shadowcode-docs` repository for relevant documentation about the feature area
...
5. Start ShadowCode automation using `vscode_automation_start`
...
## Pointers for Controlling ShadowCode

- **Prefer `vscode_automation_*` tools over `browser_*` tools** when available - these are designed specifically for ShadowCode interactions and provide more reliable control. For example:
...
**Monaco editors (used throughout ShadowCode) DO NOT work with standard Playwright methods like `.click()` on textareas or `.fill()` / `.type()`**
...
1. Gather context:
   ...
   - Search shadowcode/shadowcode-docs for documentation on the affected feature areas
