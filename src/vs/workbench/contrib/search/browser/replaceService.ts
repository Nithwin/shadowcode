...
class ReplacePreviewModel extends Disposable {
	...
	async resolve(replacePreviewUri: URI): Promise<ITextModel> {
		...
		this._register(fileMatch.onDispose(() => replacePreviewModel.dispose())); // TODO@Sandeep we should not dispose a model directly but rather the reference (depends on https://github.com/shadowcode/shadowcode/issues/17073)
