...
export class HuggingFaceLanguageModelProvider implements ILanguageModelChatProvider, IDisposable {
	...
	async sendChatRequest(modelId: string, messages: IChatMessage[], from: ExtensionIdentifier | undefined, options: ILanguageModelChatRequestOptions, token: CancellationToken): Promise<ILanguageModelChatResponse> {
		...
		const model = modelId.replace('huggingface:', '');
		...
		const context = await this.requestService.request({
			...
			callSite: 'huggingface'
		}, token);

		if (context.res.statusCode !== 200) {
			throw new Error(`Hugging Face request failed with status ${context.res.statusCode}`);
		}
			...
			listenStream(context.stream, {
				onData: chunk => {
					...
					for (const line of lines) {
						...
						try {
							const delta = JSON.parse(payload) as IHuggingFaceDelta;
							...
							this.logService.error('[HuggingFace] Failed to parse stream payload', error, payload);
						}
					}
				},
				onError: error => {
