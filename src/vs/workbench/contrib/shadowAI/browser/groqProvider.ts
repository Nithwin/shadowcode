...
export class GroqLanguageModelProvider implements ILanguageModelChatProvider, IDisposable {
			...
			listenStream(context.stream, {
				onData: chunk => {
					...
					for (const line of lines) {
						...
						try {
							const delta = JSON.parse(payload) as IGroqDelta;
							...
							this.logService.error('[Groq] Failed to parse stream payload', error, payload);
						}
					}
				},
				onError: error => {
