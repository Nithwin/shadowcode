		...
		const promptResult = await this.dialogService.prompt({
			...
			custom: {
				...
				markdownDetails: [{
					...
					markdown: new MarkdownString(`[${localize('autoApprove.markdown2', 'Learn more about the potential risks and how to avoid them.')}](https://docs.shadowcode.dev/docs/copilot/security#_security-considerations)`)
				}],
			}
		});
