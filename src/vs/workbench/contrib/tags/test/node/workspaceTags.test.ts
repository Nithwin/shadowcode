...
suite('Telemetry - WorkspaceTags', () => {
	...
	test('Multiple remotes hashed', async function () {
		...
		assert.deepStrictEqual(await getHashedRemotesFromConfig(config), [hash('github.com/shadowcode/shadowcode.git'), hash('git.example.com/gitproject.git')]);

		// Strip .git
		assert.deepStrictEqual(await getHashedRemotesFromConfig(config, true), [hash('github.com/shadowcode/shadowcode'), hash('git.example.com/gitproject')]);
