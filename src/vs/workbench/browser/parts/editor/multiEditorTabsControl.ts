...
export class MultiEditorTabsControl extends EditorTabsControl {
	...
	private updateTabSizing(fromEvent: boolean): void {
		...
		if (options.tabSizing === 'fixed') {
			...
			// For https://github.com/shadowcode/shadowcode/issues/40290 we want to
	...
	private registerTabsContainerListeners(tabsContainer: HTMLElement, tabsScrollbar: ScrollableElement): void {
		...
		// Prevent auto-scrolling (https://github.com/shadowcode/shadowcode/issues/16690)
		...
		// Prevent auto-pasting (https://github.com/shadowcode/shadowcode/issues/201696)
		if (isLinux) {
		...
		this._register(addDisposableListener(tabsContainer, EventType.MOUSE_WHEEL, (e: WheelEvent) => {
			...
			// Ignore event if the last one happened too recently (https://github.com/shadowcode/shadowcode/issues/96409)
	...
	private registerTabListeners(tab: HTMLElement, tabIndex: number, tabsContainer: HTMLElement, tabsScrollbar: ScrollableElement): IDisposable {
		...
		const handleClickOrTouch = async (e: MouseEvent | GestureEvent, preserveFocus: boolean): Promise<void> => {
			...
			if (isMouseEvent(e) && (e.button !== 0 /* middle/right mouse button */ || (isMacintosh && e.ctrlKey /* macOS context menu */))) {
				if (e.button === 1) {
					e.preventDefault(); // required to prevent auto-scrolling (https://github.com/shadowcode/shadowcode/issues/16690)
				}
		...
		disposables.add(addDisposableListener(tab, EventType.AUXCLICK, e => {
			if (e.button === 1 /* Middle Button*/) {
				EventHelper.stop(e, true /* for https://github.com/shadowcode/shadowcode/issues/56715 */);
		...
		}, true /* use capture to fix https://github.com/shadowcode/shadowcode/issues/19145 */));
	...
	private isSupportedDropTransfer(e: DragEvent): boolean {
		...
		if (e.dataTransfer && e.dataTransfer.types.length > 0) {
			return true; // optimistically allow external data (// see https://github.com/shadowcode/shadowcode/issues/25789)
		}
	...
	private redrawTabLabel(editor: EditorInput, tabIndex: number, tabContainer: HTMLElement, tabLabelWidget: IResourceLabel, tabLabel: IEditorInputLabel): void {
		...
		if (tabLabel.ariaLabel) {
			...
			// More details https://github.com/shadowcode/shadowcode/issues/95378
		...
		if (this.groupsView.partOptions.wrapTabs) {
			...
			const lastTabFitsWrapped = () => {
				...
				if (lastTabOverlapWithToolbarWidth > 1) {
					...
					// https://github.com/shadowcode/shadowcode/issues/116385
					return false;
				}
			...
			// flickering issue https://github.com/shadowcode/shadowcode/issues/115050)
	...
	private doLayoutTabsNonWrapping(options?: IMultiEditorTabsControlLayoutOptions): void {
		...
		if (
			...
			(!dimensionsChanged && !options?.forceRevealActiveTab) 	// dimensions did not change and we have low layout priority (https://github.com/shadowcode/shadowcode/issues/133631)
		) {
...
registerThemingParticipant((theme, collector) => {
	...
	// - if we have a contrast border (which draws an outline - https://github.com/shadowcode/shadowcode/issues/109117)
	// - on Safari (https://github.com/shadowcode/shadowcode/issues/108996)
