...
export abstract class AbstractFileDialogService implements IFileDialogService {
	...
	protected getPickFileToSaveDialogOptions(defaultUri: URI, availableFileSystems?: string[]): ISaveDialogOptions {
		...
		const registeredLanguageFilters: IFilter[] = coalesce(registeredLanguageNames.map(({ languageName, languageId }) => {
			...
			// https://github.com/shadowcode/shadowcode/issues/115860
			...
			if (!matchingFilter && extensions.includes(extOrPlaintext)) {
				...
				// If it isn't, add it manually. https://github.com/shadowcode/shadowcode/issues/147657
		...
		// (https://github.com/shadowcode/shadowcode/issues/96283)
		if (!matchingFilter && ext) {
			matchingFilter = { name: trim(ext, '.').toUpperCase(), extensions: [trim(ext, '.')] };
		}

		// Order of filters is
		// - All Files (we MUST do this to fix macOS issue https://github.com/shadowcode/shadowcode/issues/102713)
