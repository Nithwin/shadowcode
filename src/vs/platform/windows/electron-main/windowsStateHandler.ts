...
export class WindowsStateHandler extends Disposable {
	...
	private registerListeners(): void {
		...
		// shutting down the application (https://github.com/shadowcode/shadowcode/issues/87171)
		...
		if (windowConfig?.newWindowDimensions) {
			if (windowConfig.newWindowDimensions === 'maximized') {
				...
				if (lastActiveState.mode === WindowMode.Fullscreen) {
					state.mode = WindowMode.Fullscreen; // only take mode (fixes https://github.com/shadowcode/shadowcode/issues/19331)
				} else {
					state = {
						...lastActiveState,
