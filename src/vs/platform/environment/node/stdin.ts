...
async function createStdInFile(targetPath: string) {
	...
	await fs.promises.chmod(targetPath, 0o600); // Ensure the file is only read/writable by the user: https://github.com/shadowcode/shadowcode-remote-release/issues/9048
}
...
export async function readFromStdin(targetPath: string, verbose: boolean, onEnd?: Function): Promise<void> {
	...
	let [encoding, iconv] = await Promise.all([
		...
		createStdInFile(targetPath) 			// make sure file exists right away (https://github.com/shadowcode/shadowcode/issues/155341)
	]);
	...
	// file descriptor.
	// (https://github.com/shadowcode/shadowcode/issues/148952)
