	...
	constructor(container: HTMLElement, options: IBaseDropdownOptions) {
		...
		for (const event of [EventType.MOUSE_DOWN, GestureEventType.Tap]) {
			this._register(addDisposableListener(this._label, event, e => {
				if (isMouseEvent(e) && e.button !== 0) {
					// prevent right click trigger to allow separate context menu (https://github.com/shadowcode/shadowcode/issues/151064)
					return;
				}
		...
		this._register(addDisposableListener(this._label, EventType.KEY_DOWN, e => {
			...
			if (event.equals(KeyCode.Enter) || event.equals(KeyCode.Space)) {
				EventHelper.stop(e, true); // https://github.com/shadowcode/shadowcode/issues/57997
